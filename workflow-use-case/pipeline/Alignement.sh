...
time \
    $BWA_DIR/bwa mem \
        -K 100000000 -t 8 -Y $b38_REF \
        -R "@RG\tID:$SM\tLB:$SM\tSM:$SM\tPL:ILLUMINA" \
        $SM.R1.fq.gz $SM.R2.fq.gz \
    2>> $BAM/$SM/$SM-Alignment.log \
...
echo -e "$SM_DedupBQSR|bash $SOFT/bin/alignment-scripts/Dedup-BQSR.sh|$SM $BAM/$SM/$SM.sorted.cram" > $wrk/bqsr-task.txt
tasktide \
    manager \
