...
mkdir -p $DATA_DIR/logs/Alignment/$SM
set -e 
exec > >(tee -a $DATA_DIR/logs/Alignment/$SM/$SM-Alignment.log) 2>&1


# Setup working directory
set -x
wrk=$DATA_DIR/alignment/$SM
mkdir -p $wrk
cd $wrk
...
# Enqueue next step
echo -e "\\n\\nEnqueueing next step 'DedupBQSR' for '$SM'"
echo -e "${SM}_DedupBQSR|bash $SOFT/bin/alignment-scripts/Dedup-BQSR.sh|$SM $BAM/$SM/$SM.sorted.cram" > $wrk/bqsr-task.txt
tasktide \
    manager \
        ...
        --step-name "DedupBQSR" \
        --delimiter '|' \
        --target-file $wrk/bqsr-task.txt
rm -f $wrk/bqsr-task.txt
